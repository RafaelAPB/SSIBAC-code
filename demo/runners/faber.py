...
class TecnicoAgent(DemoAgent):
    def __init__(
        self, http_port: int, admin_port: int, no_auto: bool = False, **kwargs
    ):
        super().__init__(
            "Tecnico Lisboa Agent",
            http_port,
            admin_port,
            prefix="Tecnico Lisboa",
            extra_args=[]
            if no_auto
    ...
    try:
        ...
        with log_timer("Publish schema/cred def duration:"):
            ...
            (
                ...
                "degree schema",
                version,
                ["firstName", "lastName", "age", "id", "timestamp", "university", "type", "name", "EQF", "course", "grade", "gradeScale", "skills", "degreeId"],
        ...
        async for option in prompt_loop(options):
            if option is None or option in "xX":
                ...
                req_attrs = [
                    ...
                    {"name": "EQF", "restrictions": [{"issuer_did": agent.did}]},
                ]
                if revocation:
                    req_attrs.append(
                        {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Runs a Tecnico demo agent.")
