...
class AliceAgent(DemoAgent):
    ...
    async def handle_present_proof(self, message):
        with log_timer2("EVALUATION| Construct proof to Tecnico:"):
            state = message["state"]
    ...
    try:
        ...
        async for option in prompt_loop(
            ...
            if option is None or option in "xX":
                ...
                with log_timer2("Generate invitation duration:"):
                    # Generate an invitation
                    log_status(
                        "#7 Create a connection to alice and print out the invite details"
                    )
                    connection = await agent.admin_POST("/connections/create-invitation")
                log_msg(json.dumps('CONNECTION'), label="Invitation:", color="red")
                print(connection)
                agent.connection_id = connection["connection_id"]
                log_json(connection, label="Invitation response:")
                log_msg("*****************")
                ...
                log_msg(json.dumps('INBOUND CONNECTION WILL BE CHANGED TO QUALICHAIN UPPON ACCEPTANCE'), label="Change:", color="red")
