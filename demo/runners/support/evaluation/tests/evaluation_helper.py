...
class EvaluationHelper:
    ...
    def print_all_mean(self):
        print("Total Runtime: ", self.get_total_time_mean())
        print("Register DIDs: ", self.get_register_did_startup_phase_duration())
        print("Init Agents: ", self.get_init_agents_startup_phase_duration())
        print("Publish Schema Duration: ", self.get_publish_schema_startup_phase_duration())
        print("Connect Alice-IST: ", self.get_conn_alice_tecnico_connect_phase_duration())
        print("Emit Credential: ", self.get_emit_credential_connect_phase_duration())
        print("Connect Alice-Qualichain: ", self.get_conn_alice_qualichain_connect_phase_duration())
        print("Construct Proof Request", self.get_handle_proof_from_alice_ac_phase_duration())
        print("Construct proof to Tecnico", self.get_construct_proof_to_tecnico_ac_phase_duration())
        print("Handling Presentation Proof", self.get_handle_proof_from_alice_ac_phase_duration())
        print("Evaluating Access Control Request ", self.get_eval_access_control_ac_phase_duration())
    ...
    def get_ac_phase_duration(self):
        return [self.construct_proof_request_to_alice[i] + self.construct_proof_to_tecnico[i] + self.handle_proof_from_alice[i] + self.eval_access_control[i] for i in range(0,10)]
    ...
    def get_total_time(self):
        time = [self.get_ac_phase_duration()[i] for i in range (0,10)]
        time = time + time + time + time + time + time + time + time + time + time
        return [self.get_startup_phase_duration()[i] + self.get_connect_phase_duration()[i] + time[i] for i in range(self.get_dimensions())]
