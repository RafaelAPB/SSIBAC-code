...
class DurationTimer:
    def __init__(self, label: str = None, callback=None, callback2=None):
...
def log_timer2(label: str, show: bool = True, logger=None, **kwargs):
    logger = logger or log_msg
    f = open("eval", "a")
    cb = (
        (
            lambda timer: timer.last_error
            or logger(timer.label, f"{timer.duration:.4f}s", **kwargs)
        )
        if show
        else None
    )
    ...
    return DurationTimer(label, cb,cb2)
